...
test.skip("pro renders carry no footer", async (t) => {
...
test.skip("the free tier finalizes exactly ten; the eleventh is refused with the upgrade text and nothing written", async (t) => {
  const { box, c } = open(t);
  await c.init();
  for (let i = 1; i <= 10; i++) {
